...
try{
 ...
 function frame(now){
   ...
   if(!paused&&!document.hidden){
     ...
     slingNodes.forEach((node,i)=>{const age=world.time-flashes[19+i],active=flashes[19+i]>0&&age<.4;node.style.opacity=active?String(1-age/.4):'0';node.style.transform=active?'scale('+(1+age*1.4)+')':'';});
